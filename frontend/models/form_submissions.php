...
class FMModelForm_submissions {
   ...
   * @return array
   */
  function showsubmissions($params) {
    $form_id = $params['id'];
    ...
    $filter_order_Dir = ((isset($_POST['asc_or_desc']) && ($_POST['asc_or_desc'] == 'asc' || $_POST['asc_or_desc'] == 'desc')) ? esc_html($_POST['asc_or_desc']) : 'desc');
    $where = array();
  ...
  function show_stats() {
    ...
    $id = WDW_FM_Library::get('stat_id', 0);
    $from = WDW_FM_Library::get('startdate', '');
    $to = WDW_FM_Library::get('enddate', '');
   ...
   */
  function paypal_info( $submission_id ) {
    global $wpdb;
  ...
  public function submissions_to_export() {
    ...
    $users = array_slice($users, 0, count($users) - 1);
    $allow_export = FALSE;
    $msg = __('You have no permissions to download', WDFM()->prefix);
    if ( !is_user_logged_in() ) {
      if ( !in_array('guest', $users) ) {
    ...
    $checked_ids = WDW_FM_Library::get('checked_ids', '');
    $from = WDW_FM_Library::get('form', '');
    $to = WDW_FM_Library::get('to', '');
    $form_id = $id;
