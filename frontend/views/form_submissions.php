...
class FMViewForm_submissions {
  ...
  public function display($params) {
    if ( $params === FALSE ) {
      echo WDW_FM_Library::message(__('You have no permission to view submissions.', WDFM()->prefix), 'fm-notice-success');

      return;
    }
    wp_enqueue_style('fm-submissions_css');
    ...
    $order_by = WDW_FM_Library::get('order_by', 'group_id');
    $asc_or_desc = WDW_FM_Library::get('asc_or_desc', 'desc');
    ...
    <form action="<?php echo $current_url; ?>" method="post" name="adminForm" id="adminForm">
          ...
          <div>
            ...
            if ( $entries ) {
              ...
              <div class="reports" style="width: 80px;">
                <strong><?php _e('Entries', WDFM()->prefix); ?></strong><br />
            ...
            if ( $views ) {
              ?>
              <div class="reports" style="width: 80px;">
                <strong><?php _e('Views', WDFM()->prefix); ?></strong><br />
            ...
            if ( $conversion_rate ) {
              ...
              <div class="reports" style="width: 130px;">
                <strong><?php _e('Conversion Rate', WDFM()->prefix); ?></strong><br />
                <?php
                if ( $total_views ) {
            ...
            <div class="search_and_pagination">
              ...
              if ( $search ) {
                ...
                <div>
                  ...
                  <img id="filter_img" src="<?php echo WDFM()->plugin_url . '/images/filter_show.png'; ?>" width="40" style="vertical-align: middle; cursor: pointer;" onclick="show_hide_filter()" title="<?php _e('Search by fields', WDFM()->prefix); ?>" />
                  <input type="button" onclick="this.form.submit();" style="vertical-align: middle; cursor: pointer;" value="<?php _e('GO', WDFM()->prefix); ?>" />
                  <input type="button" onclick="remove_all();this.form.submit();" style="vertical-align:middle; cursor:pointer" value="<?php _e('Reset', WDFM()->prefix); ?>" />
                </div>
                <div>
                  <?php
                  if ( $join_count ) {
                    echo sprintf(__('%s of %s submissions are not shown, as the field you sorted by is missing in those submissions.', WDFM()->prefix), ($total_entries - $join_count), $total_entries);
                  }
                  ?>
                </div>
                <?php
            ...
            if ( $submit_date ): ?>
              <th class="submitdate_fc" style="text-align:left; <?php if ( !(strpos($lists['hide_label_list'], '@submitdate@') === FALSE) ) {
                ...
                <table class="simple_table">
                  <tr>
                    <td><?php echo __('From', WDFM()->prefix); ?>:</td>
                    <td>
                  ...
                  <tr>
                    <td><?php echo __('To', WDFM()->prefix); ?>:</td>
                    <td>
                ...
                switch ( $sorted_labels_type[$i] ) {
                  ...
                  case 'type_paypal_payment_status':
                    ...
                    <select name="<?php echo $form_id . '_' . $labels_id[$i]; ?>_search" id="<?php echo $form_id . '_' . $labels_id[$i]; ?>_search" onChange="this.form.submit();" value="<?php echo $lists[$form_id . '_' . $labels_id[$i] . '_search']; ?>">
                      <option value=""></option>
                      <option value="canceled"><?php echo __('Canceled', WDFM()->prefix); ?></option>
                      <option value="cleared"><?php echo __('Cleared', WDFM()->prefix); ?></option>
                      <option value="cleared by payment review"><?php echo __('Cleared by payment review', WDFM()->prefix); ?></option>
                      <option value="completed"><?php echo __('Completed', WDFM()->prefix); ?></option>
                      <option value="denied"><?php echo __('Denied', WDFM()->prefix); ?></option>
                      <option value="failed"><?php echo __('Failed', WDFM()->prefix); ?></option>
                      <option value="held"><?php echo __('Held', WDFM()->prefix); ?></option>
                      <option value="in progress"><?php echo __('In progress', WDFM()->prefix); ?></option>
                      <option value="on hold"><?php echo __('On hold', WDFM()->prefix); ?></option>
                      <option value="paid"><?php echo __('Paid', WDFM()->prefix); ?></option>
                      <option value="partially refunded"><?php echo __('Partially refunded', WDFM()->prefix); ?></option>
                      <option value="pending verification"><?php echo __('Pending verification', WDFM()->prefix); ?></option>
                      <option value="placed"><?php echo __('Placed', WDFM()->prefix); ?></option>
                      <option value="processing"><?php echo __('Processing', WDFM()->prefix); ?></option>
                      <option value="refunded"><?php echo __('Refunded', WDFM()->prefix); ?></option>
                      <option value="refused"><?php echo __('Refused', WDFM()->prefix); ?></option>
                      <option value="removed"><?php echo __('Removed', WDFM()->prefix); ?></option>
                      <option value="returned"><?php echo __('Returned', WDFM()->prefix); ?></option>
                      <option value="reversed"><?php echo __('Reversed', WDFM()->prefix); ?></option>
                      <option value="temporary hold"><?php echo __('Temporary hold', WDFM()->prefix); ?></option>
                      <option value="unclaimed"><?php echo __('Unclaimed', WDFM()->prefix); ?></option>
                    </select>
                    <script>
          ...
          for ( $www = 0, $qqq = count($group_id_s); $www < $qqq; $www++ ) {
            ...
            <tr class="<?php echo "row$k"; ?>">
                    ...
                    if ( $t->element_label == $labels_id[$h] ) {
                      if ( strpos($t->element_value, "***map***") ) {
                        ...
                        echo '<td align="center" class="' . $labels_id[$h] . '_fc" ' . $styleStr . '><a class="thickbox-preview" href="' . add_query_arg(array(
                                                                                                                                                           ...
                                                                                                                                                           'height' => '550',
                                                                                                                                                           'TB_iframe' => '1'
                                                                                                                                                         ), admin_url('admin-ajax.php')) . '" title="' . __('Show on Map', WDFM()->prefix) . '">' . __('Show on Map', WDFM()->prefix) . '</a></td>';
                      }
                      else {
                        if ( strpos($t->element_value, "*@@url@@*") ) {
                          ...
                          if ( strpos($t->element_value, "@@@") > -1 || $t->element_value == "@@@" || $t->element_value == "@@@@@@@@@" || $t->element_value == "::" || $t->element_value == ":" || $t->element_value == "--" ) {
                            ...
                            if ( strpos($t->element_value, "***grading***") ) {
                              ...
                              $items .= __('Total', WDFM()->prefix) . ": " . $total;
                              ...
                              if ( strpos($t->element_value, "***matrix***") ) {
                                echo '<td align="center" class="' . $labels_id[$h] . '_fc" ' . $styleStr . '><a class="thickbox-preview" href="' . add_query_arg(array(
                                                                                                                                                                   ...
                                                                                                                                                                   'height' => '550',
                                                                                                                                                                   'TB_iframe' => '1'
                                                                                                                                                                 ), admin_url('admin-ajax.php')) . '" title="' . __('Show Matrix', WDFM()->prefix) . '">' . __('Show Matrix', WDFM()->prefix) . '</a></td>';
                              }
                              else {
              ...
              <a class="thickbox-preview" href="' . add_query_arg(array(
                  ...
                  <img src="' . WDFM()->plugin_url . '/images/info.png" />
                </a></td>';
              }
              ?>
            </tr>
      ...
      if ( $stats ) {
        ...
        if ( $is_stats ) {
          ?>
          <br /><br />
          <h1 style="border-bottom: 1px solid; color:#000 !important;"><?php _e('Statistics', WDFM()->prefix); ?></h1>
          <table class="stats">
            <tr valign="top">
              <td class="key" style="vertical-align: middle;">
                <label> <?php _e('Select a Field', WDFM()->prefix); ?>: </label>
              </td>
              <td>
                <select id="stat_id">
                  <option value=""><?php _e('Select a Field', WDFM()->prefix); ?></option>
                  <?php
            ...
            <tr valign="middle">
              <td class="key" style="vertical-align: middle;">
                <label> <?php _e('Select a Date', WDFM()->prefix); ?>: </label>
              </td>
              <td>
                <?php _e('From', WDFM()->prefix); ?>:<input class="inputbox wd-datepicker" type="text" name="startstats" id="startstats" size="10" maxlength="10" />
                <?php _e('To', WDFM()->prefix); ?>:
            ...
            <tr valign="top">
              <td class="key" style="vertical-align: middle;" colspan="2">
                <input type="button" onclick="show_stats()" style="vertical-align:middle; cursor:pointer" value="<?php _e('Show', WDFM()->prefix); ?>" />
              </td>
            </tr>
          </table>
    ...
    <script>
      jQuery(window).load(function () {
        ...
        if (typeof jQuery().fancybox !== 'undefined' && jQuery.isFunction(jQuery().fancybox)) {
          jQuery(".fm_fancybox").fancybox({            'maxWidth ': 600,
            'maxHeight': 500
          });
        }
      });
      function show_stats() {
        ...
        if (jQuery('#stat_id').val() != "") {
          ...
          jQuery('#div_stats').html("<?php echo __('Please select the field!', WDFM()->prefix); ?>")
        }
      }
      ...
      ?>
      document.getElementById('fm-fields-filter').style.display = '';
      document.getElementById('filter_img').src = '<?php echo WDFM()->plugin_url . '/images/filter_hide.png'; ?>';
      <?php
      }
      ?>
    </script>
   ...
   * @param $choices
   */
  public function show_stats($choices) {
    ...
    <table class="adminlist">
      <thead>
        <tr>
          ...
          <th width="10%"><?php _e('Count', WDFM()->prefix); ?></th>
        </tr>
      </thead>
      <?php
      $k = 0;
      ...
      <tr>
        <td colspan="2" style="text-align: right; color: #000;">
          <strong><?php _e('Total', WDFM()->prefix); ?></strong>
        </td>
        <td>
  ...
  public function show_map($long, $lat) {
    ...
    wp_print_scripts('fm-gmap_form_back');
    ?>
    <table style="margin: 0px; padding: 0px;">
      <tr>
        <td>
          <b><?php _e('Address', WDFM()->prefix); ?>:</b>
        </td>
        <td>
      ...
      <tr>
        <td>
          <b><?php _e('Longitude', WDFM()->prefix); ?>:</b>
        </td>
        <td>
      ...
      <tr>
        <td>
          <b><?php _e('Latitude', WDFM()->prefix); ?>:</b>
        </td>
        <td>
   ...
   * @param $matrix_params
   */
  public function show_matrix( $matrix_params ) {
   ...
   * @param $submission
   */
  public function paypal_info($submission) {
    if ( !isset($submission->ipn) ) {
      ...
      <div style="width: 100%; text-align: center; height: 100%; vertical-align: middle;">
        <h1 style="top: 44%; position: absolute; left: 38%; color: #000;">
          <?php _e('No information yet', WDFM()->prefix); ?>
        </h1>
      </div>
      <?php
    }
      ...
      ?>
      <h2><?php _e('Payment Info', WDFM()->prefix); ?></h2>
      <table class="admintable">
        ...
        <tr>
          <td class="key"><?php _e('Last modified', WDFM()->prefix); ?></td>
        ...
        <tr>
          <td class="key"><?php _e('Total', WDFM()->prefix); ?></td>
   ...
   * @param $form
   */
  public function generate_csv($form) {
    $title = $form['title'];
    $ptn = "/[^a-zA-Z0-9_]/";
    $rpltxt = "";
   ...
   * @param $form
   */
  public function generate_xml($form) {
    $title = $form['title'];
    $form_id = $form['id'];
    $ptn = "/[^a-zA-Z0-9_]/";
    $rpltxt = "";
